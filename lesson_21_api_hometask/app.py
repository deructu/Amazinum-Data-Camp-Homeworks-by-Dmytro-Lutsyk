...
app = FastAPI(
    ...
    description="Simple computer vision deployment homework with FastAPI and YOLO.",
    version="1.0.0",
)
...
def detect_objects(image: np.ndarray, confidence: float) -> tuple[Any, list[dict[str, Any]]]:
    """Run YOLO inference and convert detections to JSON-friendly format."""
...
async def detect(
    ...
    """Return detected objects as JSON and save an annotated image to outputs/."""
...
async def detect_image(
    ...
    """Return an annotated image with YOLO bounding boxes."""
    contents = await image.read()
    input_image = decode_image(contents)
